...
export default function ProUpgrade() {
  ...
  const handleComplete = async () => {
    if (!profile || !user || !category) return;
    ...
    try {
      ...
      // Add professional role (keep client role) — ignore duplicate-key errors
      ...
      if (roleErr && !/duplicate|unique/i.test(roleErr.message)) {
        ...
        toast.error(`Could not add professional role: ${roleErr.message}`);
        return;
      }
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center px-4">
        <div className="text-center space-y-3">
          ...
          <h1 className="font-display text-2xl font-bold">Professional Mode Enabled!</h1>
          <p className="text-sm text-muted-foreground">You can now offer services and receive bookings. Taking you to your dashboard...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="w-full max-w-sm mx-auto space-y-6">
        ...
        <div className="rounded-xl bg-primary/5 border border-primary/20 p-3">
          <p className="text-xs text-muted-foreground">
            🎉 <span className="font-medium text-foreground">Upgrading to Professional</span> — Your client features (bookings, saved pros, etc.) will be preserved.
          </p>
        </div>
          ...
          <div className="space-y-5">
            ...
            <div className="flex gap-3">
              ...
              <Button className="flex-1 rounded-xl h-11" onClick={handleComplete} disabled={loading}>
                {loading ? "Setting up..." : "Enable Professional Mode"}
              </Button>
            </div>
          </div>
        )}
