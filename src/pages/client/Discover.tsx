...
import { mockProfessionals, categoryLabels } from "@/data/mock";
import { useState } from "react";
import { ArrowLeft, SlidersHorizontal, MapPin, List } from "lucide-react";
...
export default function Discover() {
  ...
  const filtered = activeFilter === "All"
    ...
    ? mockProfessionals.filter(p => ["open-chair", "available-now", "last-minute"].includes(p.status))
    : activeFilter === "Open Chair"
    ? mockProfessionals.filter(p => p.status === "open-chair")
    : mockProfessionals.filter(p => categoryLabels[p.category]?.toLowerCase().startsWith(activeFilter.toLowerCase()));
      ...
      {view === "map" ? (
        <div className="relative">
            ...
            {filtered.map((pro, i) => (
              ...
              >
                <div className={cn(
                  ...
                  pro.status === "last-minute" ? "ring-status-last-minute" :
                  "ring-border"
                )}>
                  <img src={pro.avatar} alt="" className="h-full w-full object-cover" />
                </div>
              </Link>
            ))}
          ...
          <div className="px-4 pt-4 space-y-3">
            ...
            {filtered.map(pro => (
              <ProCard key={pro.id} pro={pro} variant="compact" />
            ))}
          </div>
        </div>
      ) : (
